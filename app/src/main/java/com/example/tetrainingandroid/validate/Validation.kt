...
object Validation {
    fun isRequestTokenExpire(response: RequestTokenResponse?): Boolean {
        ...
        if (System.currentTimeMillis() - response.expiresAtSinceEpoch > Config.REQUEST_TOKEN_EXPIRES_AT) {
            return true
        }
        return false
    }
    ...
    fun isSessionExpire(response: Session?): Boolean {
        ...
        if (System.currentTimeMillis() - response.expiresAtSinceEpoch!! > Config.SESSION_EXPIRES_AT) {
            return true
        }
        return false
    }
